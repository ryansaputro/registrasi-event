        ...
        <div class="form-group col-md-4">
            ...
            <select id="id_jabatan" name="id_jabatan" class="form-control">
                @foreach($jabatan as $ref_jabatan)
                    <option value='{{$ref_jabatan->id_jabatan}}' {{$ref_jabatan->id_jabatan == $model->id_jabatan ? 'selected' : ''}}>{{$ref_jabatan->nama_jabatan}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group col-md-4">
            ...
            <select id="id_divisi" name="id_divisi" class="form-control">
                @foreach($divisi as $ref_divisi)
                    <option value='{{$ref_divisi->id_divisi}}' {{$ref_divisi->id_divisi == $model->id_divisi ? 'selected' : ''}}>{{$ref_divisi->nama_divisi}}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group col-md-4">
        ...
        <div class="form-group col-md-4">
            ...
            <select id="id_marital" name="id_marital" class="form-control">
                @foreach($marital as $ref_marital)
                    <option value='{{$ref_marital->id_marital}}' {{$ref_marital->id_marital == $model->id_marital ? 'selected' : ''}}>{{$ref_marital->status_marital}}</option>
                @endforeach
            </select>
        </div>
    </div>
        ...
        <div class="form-group col-md-4">
            ...
            <select id="id_status_aktif" name="id_status_aktif" class="form-control">
                @foreach($aktif as $ref_aktif)
                    <option value='{{$ref_aktif->id_status_aktif}}' {{$ref_aktif->id_status_aktif == $model->id_status_aktif ? 'selected' : ''}}>{{$ref_aktif->nama_status_aktif}}</option>
                @endforeach
            </select>
        </div>
    </div>
