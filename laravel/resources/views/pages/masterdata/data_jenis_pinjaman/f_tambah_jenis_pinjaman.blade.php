        ...
        <div class="form-group col-md-6">
            ...
            <select id="id_jenis_angsuran" name="id_jenis_angsuran" class="form-control">
                @foreach($angsuran as $ref_angsuran)
                    <option value='{{$ref_angsuran->id_jenis_angsuran}}'>{{$ref_angsuran->jenis_angsuran}}</option>
                @endforeach
            </select>
            <!-- {!! Form::select('id_jenis_angsuran', 
                ['1' => 'Datar (Flate)', '2' => 'Menurun (Sliding Rate)'],null,['class' => 'form-control']) !!} -->
        </div>
    </div><br>
    <div><br/>
        <div class="form-group col-md-6">
