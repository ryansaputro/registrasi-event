...
<section class="content-header">
  <h1>
    Laporan Daftar Peserta By Pembayaran
  </h1>
</section>
    ...
    <div class="col-md-12">
      <div class="box panel-primary">
        ...
        <div class="panel-footer" style="background-color:#f9f9f9;">
          <table id="data_pembayaran" class="table table-hover" style="width:100%">
              <thead>
                  <tr>
              ...
              <tbody>
                  @foreach ($data as $k => $v)
                      <tr>
                        <td>{{$v->kode_event}}</td>
                        <td>{{$v->nama_event}}</td>
                        @if(!empty($pesertaBlmByr[$v->id]))
                            <td>{{array_key_exists($v->id, $pesertaBlmByr) ? $pesertaBlmByr[$v->id] : 0}} orang</td>
                        @else
                            <td>-</td>
                        @endif
