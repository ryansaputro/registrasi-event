...
<body>
    ...
    <div class="container" style="padding-top:10px;border:2px solid #6464;">
        ...
        <div class="step_all" style="">
            ...
            <p>Terimakasih anda telah melakukan pendaftaran dan verifikasi, untuk selanjutnya sila lakukan pembayaran dengan melakukan transfer pembayaran ke no rekening <b>{{$data->no_rekening}}</b> Bank <b>{{$data->nama_bank}}</b> an <b>{{$data->pemilik_rekening}}</b>.</p>
            <p>Jika telah melakukan pembayaran, bukti transfer/pembayaran tolong diupload melalui halaman <a href="{{URL::to('/event/'.strtolower(str_replace(' ', '-', $data->kode_event)).'/'.md5($data->id_member))}}">{{md5(URL::to('/event/'.str_replace(' ', '-', $data->nama_event).'/'.$data->id_member))}}</a> atau mengirimkan bukti ke no WA {{$data->no_wa_kontak}} an {{$data->nama}}</p>
                ...
                <h4 class="step_3"><label style="background:green;color:#fff;background: #999;color: #fff;padding: 20px;border-radius: 20px;">3</label>Silahkan melakukan pembayaran di Bank <b>{{$data->nama_bank}}</b> nomor rekening <b>{{$data->no_rekening}}</b> atas nama <b>{{$data->pemilik_rekening}}</b></h4> --}}
        </div>
