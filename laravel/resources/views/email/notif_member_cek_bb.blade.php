...
<body>
    <div class="container" style="padding-top:10px;border:2px solid #6464;">
        ...
        <div class="step_all" style="padding:20px;">
            ...
            <p>BUKTI TRANSFER Peserta No {{$id_users->id}}</p>
            <p>Telah berhasil direkam ke dalam sistem.</p>
            <p>Terimakasih telah melakukan pembayaran, tunggu email pemberitahuan dari kami untuk status approval kepesertaan dari {{strtoupper($data->nama_event)}}</p>
            <p>Status approval kepesertaan dapat dilihat pada <a href="{{URL::to('/event/'.strtolower(str_replace(' ', '-', $data->kode_event)).'/'.md5($id_users->id_member))}}">{{md5(URL::to('/event/'.str_replace(' ', '-', $data->nama_event).'/'.$id_users->id_member))}}</a></p>
