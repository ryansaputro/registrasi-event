...
class RegistrasiPesertaEventController extends Controller
{
    ...
    {   
        ...
        $event = RegistrasiEventModel::where('kode_event', $kode_event)->first();
        ...
        $isFreeActive = RegistrasiEventJenisPembayaranModel::where('id_event', $event->id)->where('tanggal', '<=', date('Y-m-d'))->orderBy('tanggal', 'DESC')->limit(1)->first();
    ...
    public function registrasi($kode_event, $id_peserta, Request $request)
    {
        ...
        try {
            ...
            $this->validate($request, [
                ...
                'no_telp'               => 'required|numeric|digits:12', //tabel,atribut
            ],$messages);
            ...
            foreach($post AS $k => $v){
                ...
                $isi = 'Terima Kasih telah me-registrasi event '.$request->nama_event;
                Mail::to($id_users->user_email)->send(new SendMailable($isi));

            } catch (\Illuminate\Database\QueryException $ex) {
            DB::rollback();
            return redirect()->to('http://localhost/portalse')
